    ...
    <div className="bg-white border border-slate-200 rounded-lg p-3 shadow-xl text-xs">
      ...
      {payload.map((p: any) => (
        <div key={p.dataKey} className="flex items-center gap-2 py-0.5">
          ...
          <span className="text-slate-600 w-24">{LAYER_LABELS[p.dataKey]}</span>
...
export default function SimilarityView({ file }: Props) {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-4">
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          <h2 className="text-sm font-semibold text-slate-900 flex items-center gap-2">
            ...
            유사도 추이 차트
            {results.length > 0 && (
              <span className="text-xs font-normal text-slate-500">{results.filter(r=>r.prev_sha!=='').length}개 비교점</span>
            )}
          </h2>
          <p className="text-xs text-slate-500 mt-0.5">낮을수록 큰 변경 · 1.0 = 완전 동일</p>
        </div>
        {/* 레이어 범례 */}
      ...
      {!loading && !error && results.length > 0 && (
        <>
          ...
          {chartData.length > 0 ? (
            ...
            <p className="text-xs text-slate-500 text-center py-6">비교 가능한 커밋 쌍이 없습니다.</p>
          )}

          {/* 커밋별 테이블 (접기 가능) */}
          <details className="rounded-lg border border-slate-200">
            <summary className="text-xs text-slate-500 font-medium px-4 py-2.5 cursor-pointer select-none hover:text-slate-600 bg-slate-50 rounded-lg">
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-slate-100 border-b border-slate-200">
                    <th className="text-left px-3 py-2 text-slate-600 font-medium w-20">커밋</th>
                    <th className="text-left px-3 py-2 text-slate-600 font-medium">메시지</th>
                    <th className="text-center px-3 py-2 text-slate-600 font-medium w-16">변경</th>
                  ...
                  {results.map((r, i) => {
                    ...
                    return (
                      <>
                        <tr
                          key={r.sha}
                          className={`border-b border-slate-200/60 cursor-pointer transition-colors
                            ${isOpen ? 'bg-slate-100/95' : 'hover:bg-slate-100/20'}`}
                          onClick={() => setSelectedRow(isOpen ? null : i)}
                        >
                          <td className="px-3 py-2"><span className="font-mono text-blue-400">{r.sha_short}</span></td>
                          <td className="px-3 py-2 text-slate-700 max-w-xs truncate">{r.message}</td>
                          <td className="px-3 py-2 text-center">
                            {r.changed ? <span className="text-amber-400">●</span> : <span className="text-slate-600">–</span>}
                          </td>
                          {isFirst ? (
                            <td colSpan={4} className="px-3 py-2 text-center text-slate-500">최초 커밋</td>
                          ) : (
                            (['L1','L2','L3','L4'] as const).map(layer => (
                        ...
                        {isOpen && !isFirst && (
                          <tr key={`${r.sha}-d`} className="bg-slate-50/70">
                              ...
                              <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
                                {(['L1','L2','L3','L4'] as const).map(layer => (
                                  <div key={layer} className="rounded-lg border p-2"
                                    ...
                                    <p className="text-xs text-slate-500 mt-1 leading-tight">{LAYER_DESC[layer]}</p>
                                  </div>
                                ))}
                              </div>
                            </td>
      ...
      {!loading && !error && !file && (
        <p className="text-slate-500 text-sm text-center py-8">파일을 선택해주세요.</p>
      )}
    </div>
  )
}
